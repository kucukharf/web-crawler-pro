# web-crawler
It crawl websites, runs on terminal for archive usage and uses nodejs
...
npm run crawl --url=http://www.sony.co.uk/all-electronics --max-old-space-size=4096

## Run

Example 

- `npm run crawl --url=http://www.sony.com/all-electronics`

Inspect with Google Chrome

node --inspect --debug-brk index.js

Debugger will listen on a random port
(Warning: This is an experimental feature and could change at any time.)
To start debugging, open the following URL in Chrome:

chrome-devtools://devtools/remote/serve_file/@API_KEY/inspector.html?experiments=true&v8only=true&ws=@HOST:@PORT/@TOKEN
...
Default options you can find in [lib/config/defaults.js](https://github.com/kucukharf/web-crawler/blob/master/lib/config/defaults.js) or get them using `scrape.defaults`.
...
```
Scrape function resolves with array of [Resource](https://github.com/kucukharf/web-crawler/blob/master/lib/resource.js) objects which contain `metadata` property from `httpResponseHandler`. 

#### resourceSaver
Class which saves [Resources](https://github.com/kucukharf/web-crawler/blob/master/lib/resource.js), should have methods `saveResource` and `errorCleanup` which return Promises. Use it to save files where you need: to dropbox, amazon S3, existing directory, etc. By default all files are saved in local file system to new directory passed in `directory` option (see [lib/resource-saver/index.js](https://github.com/kucukharf/web-crawler/blob/master/lib/resource-saver/index.js)).
```javascript
scrape({
  urls: ['http://example.com/'],
  directory: '/path/to/save',
...
#### onResourceSaved
Function called each time when resource is saved to file system. Callback is called with [Resource](https://github.com/kucukharf/web-crawler/blob/master/lib/resource.js) object. Defaults to `null` - no callback will be called.
```javascript
scrape({
  urls: ['http://example.com/'],
  directory: '/path/to/save',
...
#### onResourceError
Function called each time when resource's downloading/handling/saving to fs was failed. Callback is called with - [Resource](https://github.com/kucukharf/web-crawler/blob/master/lib/resource.js) object and `Error` object. Defaults to `null` - no callback will be called.
```javascript
scrape({
  urls: ['http://example.com/'],
  directory: '/path/to/save',
...
Callback function, optional, includes following parameters:
  ...
  - `result`: if error - `null`, if success - array of [Resource](https://github.com/kucukharf/web-crawler/blob/master/lib/resource.js) objects containing:
